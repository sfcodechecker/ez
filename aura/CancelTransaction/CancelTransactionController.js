({
			...
			if(state === 'SUCCESS'){
				...
				if(result.Q_Charge__Gateway_Name__c == 'nab transact'){
					if(result.Q_Charge__Transaction_Status__c == 'Cancelled'){
						...
						toastEvent.setParams({"type":"warning","message": component.get('v.TransactionCancelled')});
						toastEvent.fire();
						$A.get("e.force:closeQuickAction").fire();
					}
					else if(result.Q_Charge__Transaction_Status__c == 'Paid'){
						var toastEvent = $A.get("e.force:showToast");
						toastEvent.setParams({"type":"warning","message": component.get('v.TransactionPaid')});
						toastEvent.fire();
						$A.get("e.force:closeQuickAction").fire();
					}
					else if(result.Q_Charge__Transaction_Status__c == 'Scheduled'){
						var toastEvent = $A.get("e.force:showToast");
						toastEvent.setParams({"type":"warning","message": component.get('v.CancelScheduledTransaction')});
						toastEvent.fire();
						$A.get("e.force:closeQuickAction").fire();
					}
						...
						checkIsConnected.setCallback(this,function(response){
								...
								if(state === "SUCCESS"){
									if(result == true){
										...
										var toastEvent = $A.get("e.force:showToast");
										toastEvent.setParams({"type":"warning","message": component.get('v.CancelTransactionOnDisconnectedGateway')});
										toastEvent.fire();
										return;
                                    }
								}
	...
	confirm : function(component, event, helper){
		console.log('In confirm method');
		component.set('v.showSpinner',true);
		helper.cancelTransaction(component, event);
	},
